...
def _perp_flare_space() -> list[Param]:
    ...
    return [
        ...
        FloatRange("maintenance_margin_rate", 0.02, 0.15, decimals=5),
...
def _validate_perp_flare_params(params: dict[str, Any]) -> bool:
    try:
        ...
        if float(params["max_leverage"]) <= 0:
            return False
        if float(params["maintenance_margin_rate"]) <= 0:
            return False
        return True
    except Exception:
        return False
            ...
            for spec in space:
                ...
                try:
                    if isinstance(spec, IntRange):
                        ...
                        params[spec.name] = int(max(lo, min(hi, int(params[spec.name]))))
                    else:
                        base_f = float(base)
                        lo = max(spec.lo, float(base_f * (1.0 - drift_frac)))
                        hi = min(spec.hi, float(base_f * (1.0 + drift_frac)))
                        params[spec.name] = float(max(lo, min(hi, float(params[spec.name]))))
                except Exception:
                    continue
...
class ObjectiveConfig:
    ...
    w_total_return: float = 1.0
    w_sharpe: float = 0.05
    w_positive_trading_days: float = 0.10
    w_drawdown: float = 0.75  # penalty on abs(max_drawdown)
    w_turnover: float = 0.002  # penalty on turnover
    w_worst_day: float = 0.35  # penalty on abs(worst_day_return)
...
class WalkForwardSegment:
    ...
    def to_dict(self) -> dict[str, dict[str, str]]:
        return {"train": self.train.to_dict(), "validate": self.validate.to_dict(), "test": self.test.to_dict()}
...
def _read_run_stats(run_dir: Path) -> BacktestStats:
    ...
    if len(trades):
        tts = pd.to_datetime(trades["timestamp"], errors="coerce", utc=True)
        tts = tts.dropna()
        trading_days = set(tts.dt.date)
    else:
        trading_days = set()
    ...
    if len(trades) and "strategy_reason" in trades.columns:
        liquidation_count = int((trades["strategy_reason"].astype(str) == "LIQUIDATION").sum())
...
def score_run(run_dir: Path, *, objective: ObjectiveConfig) -> ScoreResult:
    stats = _read_run_stats(run_dir)
    ...
    breakdown = {
        ...
        "positive_trading_days": float(stats.positive_trading_day_frac) * float(objective.w_positive_trading_days),
    ...
    return ScoreResult(score=score, rejected=False, reason="", stats=stats, breakdown=breakdown)
...
def _run_backtest_for_market(
    *,
    ...
    return run_backtest(bars_by_symbol=bars_by_symbol, strategy=strat, cfg=cfg, run_dir=run_dir)
...
def tune_walk_forward(
    *,
    ...
    segments = build_walk_forward_segments(start=start_ts, end=end_ts, cfg=tune_cfg.walk_forward)

    run_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "segments.json").write_text(json.dumps([s.to_dict() for s in segments], indent=2))
    (run_dir / "config.json").write_text(json.dumps(asdict(tune_cfg), indent=2, default=str))
        ...
        for seg_i, seg in enumerate(segments):
            ...
            if incumbent:
                ...
                try:
                    ...
                    scored = score_run(seg_tmp, objective=tune_cfg.objective)
                    incumbent_selection_score = float(scored.score)
                finally:
            ...
            for trial_i in range(int(tune_cfg.trials_per_segment)):
                ...
                try:
                    train_bars = _slice_bars(bars_by_symbol, start=seg.train.start, end=seg.train.end)
                    _run_backtest_for_market(
                        market=market_enum,
                    ...
                    train_score = score_run(train_dir, objective=tune_cfg.objective)
                    ...
                    val_score = score_run(val_dir, objective=tune_cfg.objective)

                    # Selection score: favor validation, but require it isn't a train-only mirage.
                    selection_score = (
                        0.25 * float(train_score.score) + 0.75 * float(val_score.score)
                    )
                    ...
                    if on_progress is not None:
                        on_progress(
                            TuneProgress(
                                ...
                                last_reject_reason=str(train_score.reason or val_score.reason or ""),
                            )
                        )
                finally:
            ...
            test_bars = _slice_bars(bars_by_symbol, start=seg.test.start, end=seg.test.end)
            _run_backtest_for_market(
                market=market_enum,
            ...
            test_score = score_run(test_run_dir, objective=tune_cfg.objective)
