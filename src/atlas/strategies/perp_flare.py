...
class PerpFlare(Strategy):
    ...
    def target_exposures(
        ...
        state: dict,  # Contains 'equity', 'positions', 'funding_rates'
    ) -> StrategyDecision:
        
        # 0. Prep
        equity = state.equity
        positions = state.positions
        current_funding = state.extra.get("funding_rates", {})
        
        target_qtys = {}
        stop_losses = {} # Not universally supported by backend yet, but we calculate them
        decision_meta = {}
        ...
        for sym in self.symbols:
            ...
            friction = 2 * (self.half_spread_bps + self.base_slippage_bps + self.taker_fee_bps) + \
                       100 * abs(funding_rate) * 10000 
        ...
        target_qtys = {s: 0.0 for s in self.symbols} # Default flatten if no signal? 
        ...
        if candidates:
            ...
            # Max Exposure Cap
            max_notional = equity * self.max_leverage * self.max_margin_utilization
